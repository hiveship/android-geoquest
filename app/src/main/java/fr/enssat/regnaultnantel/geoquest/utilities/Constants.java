package fr.enssat.regnaultnantel.geoquest.utilities;

public final class Constants {

    public static final int SECURITY_DISTANCE_METERS = 20;
    public static final int DISTANCE_NEAR_METERS = 250;
    public static final int DISTANCE_MID_METERS = 600;

    public static final int DEFAULT_MAP_ZOOM = 15;
    public static final String ITINERARY_INTENT_PARAM = "itinerary";
    public static final String GEO_QUEST_SD_CARD_DIRECTORY = "/geoquest";

    public static final String DEFAULT_ITINERARY_NAME = "Example";
}
